...
const login = async (req, res, next) => {
  try {
    ...
    if (
      ...
      logger.warn(
        `Either username(${username}) does not exists or password is invalid`
      );
...
const getNewToken = async (req, res, next) => {
  ...
  try {
    ...
    const { user_id, username, name, user_type } = jwtServices.verify(
    ...
    //generate new tokens
    let new_access_token = jwtServices.sign(
      { user_id, username, name, user_type },
      JWT_SECRET,
      JWT_EXPIRY
    );
    let new_refresh_token = jwtServices.sign(
      { user_id, username, name, user_type },
      REFRESH_SECRET,
      REFRESH_EXPIRY
    );
