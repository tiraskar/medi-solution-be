...
const createUser = async (userDetails) => {
    let query, result
    const { name, address, contact, username, password, status, branch, created_by } = userDetails;
...
const getUserPagination = async (limit, offset, status, name, viewAll, user_id) => {
        ...
        if (name == "" || name == null) {
            ...
            query = `SELECT SQL_CALC_FOUND_ROWS 
                   ...
                   WHERE u.created_by=? AND u.status=? AND u.name LIKE ? GROUP BY u.user_id , u.name , u.address , u.contact , u.username , u.status , u.user_type
                    ORDER BY u.user_id DESC
                    LIMIT ? OFFSET ?`
            result = await con.query(query, [user_id, status, name + "%", limit, offset])
            totalCount = await con.query('SELECT FOUND_ROWS() as total')
        }
        let obj = { data: result[0], total: totalCount[0] }
...
const updateUser = async (userObj, userLogDetails, user_type) => {
    ...
    await executeTransaction(async (con) => {
        ...
        if (branch.length > 0) {
            for (let i = 0; i < branch.length; i++) {
                const branchExist = checkIdExists.some(idObj => idObj.branch_id === branch[i]);
